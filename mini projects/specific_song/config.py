...
ASSETS_DIR = Path(os.environ["SONGS_ASSETS_DIR"])
...
# After typing "ili", press one of these keys within MANUAL_TRIGGER_WINDOW seconds
# to play a song instantly — no voice recording needed.
# Set each value to the song's OBS source stem (without OBS_SOURCE_PREFIX), or "" to leave unassigned.
# Note: "V" is the "send now" stop key during active recording. Manual trigger
# keys here are only active during the MANUAL_TRIGGER_WINDOW before recording starts.
MANUAL_TRIGGER_SONGS: dict[str, str] = {
    "!": "",   # ili!
    "@": "",   # ili@
    "#": "",   # ili#
    "$": "",   # ili$
...
MANUAL_TRIGGER_WINDOW: float = 0.3  # seconds after "ili" to wait for a manual key

# ── Recording ─────────────────────────────────────────────────────────────────
# Hard-cancel safety net if '!' and the 2s auto-send both fail.
RECORD_TIMEOUT_SECONDS: float = 5.0
...
# ── Bass Animation ─────────────────────────────────────────────────────────────
# Set to True to enable real-time audio-reactive animation of the OBS source.
# The visualizer decodes the mp3/mp4 file directly — no loopback device needed.
